...
class Document {
  ...
  clone(data={}, options={}) {
    ...
    if ( !options.keepId ) delete data._id;
    const context = {parent: this.parent, pack: this.pack, keepId: options.keepId};
    if ( options.save ) return this.constructor.create(data, context);
  ...
  testUserPermission(user, permission, {exact=false}={}) {
    ...
    // Get user permission
    const perms = CONST.ENTITY_PERMISSIONS;
